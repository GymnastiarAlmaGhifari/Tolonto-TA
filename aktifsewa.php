...
if ($booking->aktifsewa($id, $idadmin)) {
    $idps = $booking->getps_sewa($id);
    $fcm = $user->fetchuser($idps[0]['id_user']);
    if ($booking->aktifps_sewa($idps[0]['id_ps'])) {
        echo json_encode(['status' => 'success' ]);
        sendPush($fcm['fcm'], 'Sewa telah diaktifkan', 'Sewa Anda dengan Kode Sewa ' .$id . ' telah aktif, Selamat Bermain!', 'https://tolonto.okifirsyah.com/public/brand-logo.png', '');
    } else {
