<!-- modal Delete  start -->
<section>
    <div id="modal_overlay_delete_ps_sewa" class="hidden absolute inset-0 bg-black bg-opacity-30 h-screen w-full flex justify-center items-center pt-10 md:pt-0 z-50">
        <!-- modal -->
        <div id="modal_delete_ps_sewa" class="opacity-0 transform -translate-y-full scale-150  relative bg-neutral_050 h-[300px] xs:w-[345px] sm:w-[500px] rounded-2xl flex flex-col justify-center gap-4 transition-opacity transition-transform duration-300">
            ...
            <h2 class="mx-auto text-center xs:px-5 sm:px-0 text-neutral_900">Apakah anda benar ingin menghapus ps sewa <span class="font-semibold text-error_600" id="namaPsSewa" name="namaPsSewa"></span> ?</h2>
            <h2 class="mx-auto text-center text-base font-medium text-error_600 -mt-4"> proses ini tidak bisa dikembalikan</h2>
            <form action="inventory.php" method="post" class="flex flex-col items-center justify-center gap-2 mt-2" enctype="multipart/form-data">
                ...
                <div class="flex flex-row xs:gap-6 md:gap-[42px] mt-2 items-center justify-center w-full">
                    <button type="button" title="batal" onclick="openModalDeletePsSewa(false)" name="Batal-Rental-Ps" id="Batal-Rental-Ps" value="Batal-Rental-Ps" class="bg-neutral_050 hover:bg-neutral_200 focus:bg-neutral_400 text-neutral_900 border border-neutral_600 w-5/12 h-12 rounded-2xl shadow-elevation-light-2">
                        Batal
                    </button>
                    <button type="button" title="konfirmasi hapus ps sewa" name="Konfirmasi-delete-ps-sewa" id="Konfirmasi-delete-ps-sewa" class="bg-error_600 text-neutral_050 w-5/12 h-12 rounded-2xl shadow-elevation-light-2 hover:bg-error_300 focus:bg-error_800">Konfirmasi</button>
                </div>
            </form>
        </div>
    </div>
        ...
        button.addEventListener('click', () => {
            ...
            xhr.onreadystatechange = function() {
                if (xhr.readyState === 4 && xhr.status === 200) {
                    ...
                    document.getElementById("namaPsSewa").innerHTML = json.nama_ps;
