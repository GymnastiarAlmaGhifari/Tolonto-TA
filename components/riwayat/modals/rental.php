...
<section>
    <div id="modal_overlay_delete_rental" class="hidden absolute inset-0 bg-black bg-opacity-30 h-screen w-full flex justify-center items-center pt-10 md:pt-0 z-50">
        <!-- modal -->
        <div id="modal_delete_rental" class="opacity-0 transform -translate-y-full scale-150  relative bg-neutral_050 h-[300px]  xs:w-[345px] sm:w-[500px] rounded-2xl flex flex-col justify-center gap-4 transition-opacity transition-transform py-2 duration-300">
            ...
            <h2 class="mx-auto xs:px-5 text-center sm:px-0 text-neutral_900">Apakah anda benar ingin menghapus riwayat rental dengan ID </h2>
            <h2 class="mx-auto text-center text-base font-medium -mt-4 text-neutral_900"><span class="font-semibold text-neutral_900" id="getRental" name="getRental"></span> ?</h2>
            ...
            <form action="riwayat.php" method="post" class="flex flex-col items-center justify-center gap-2 mt-2" enctype="multipart/form-data">
                ...
                <div class="flex flex-row xs:gap-6 md:gap-[42px] mt-2 items-center justify-center w-full">
                    <button type="button" title="batal" onclick="openModalDeleteRental(false)"  class="bg-neutral_050 hover:bg-neutral_200 focus:bg-neutral_400 text-neutral_900 border border-neutral_600 w-5/12 h-12 rounded-2xl shadow-elevation-light-2">
                        Batal
                    </button>
                    <button type="button" title="konfirmasi hapus riwayat rental" name="Konfirmasi-delete-rental" id="Konfirmasi-delete-rental" class="bg-error_600 text-neutral_050 w-5/12 h-12 rounded-2xl shadow-elevation-light-2 hover:bg-error_300 focus:bg-error_800">Konfirmasi</button>
                </div>
            </form>
        </div>
    </div>
             ...
             if (xhr.readyState === 4 && xhr.status === 200) {
                 ...
                 if (json.status == "success") {
                     Swal.fire({
                         icon: 'success',
                         title: 'Berhasil',
                         text: 'Berhasil menghapus ' + rental_hapus + '',
