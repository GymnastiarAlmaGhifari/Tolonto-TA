...
<section>
    <div id="modal_overlay_delete_servis" class="hidden absolute inset-0 bg-black bg-opacity-30 h-screen w-full flex justify-center items-center pt-10 md:pt-0 z-50">
        ...
        <div id="modal_delete_servis" class="opacity-0 transform -translate-y-full scale-150  relative bg-neutral_050 h-[300px]  xs:w-[345px] sm:w-[500px] rounded-2xl flex flex-col justify-center gap-4 transition-opacity transition-transform duration-300">
            ...
            <h2 class="mx-auto xs:px-5 sm:px-0 ">Apakah anda benar ingin menghapus riwayat topup <span class="font-semibold text-error_600" id="getServis" name="getServis"></span> ?</h2>
            ...
            <form action="user.php" method="post" class="flex flex-col items-center justify-center gap-2 mt-2" enctype="multipart/form-data">
                <div class="flex flex-row xs:gap-6 md:gap-[42px] mt-2 items-center justify-center w-full">
                    <button type="button" onclick="openModalDeleteServis(false)" name="Batal-Delete-Admin" id="Batal-Delete-Admin" value="Batal-Delete-Admin" class="bg-neutral_050 text-neutral_900 border border-neutral_600 w-5/12 h-12 rounded-2xl shadow-elevation-light-2">
                        Batal
                    </button>
                    <button type="submit" name="Konfirmasi-delete-servis" id="Konfirmasi-delete-servis" class="bg-error_600 text-neutral_050 w-5/12 h-12 rounded-2xl shadow-elevation-light-2">Konfirmasi</button>
                </div>
            </form>
        </div>
    </div>
...
<script>
        ...
        button.addEventListener('click', () => {
            ...
            xhr.onreadystatechange = function() {
                if (xhr.readyState === 4 && xhr.status === 200) {
                    ...
                    document.getElementById("getServis").innerHTML = json.username;
                    konfirmasiDeleteServis.value = json.id_admin;
                }
            };
            ...
            //  document.getElementById("getServis").value = button.value;
            //  //  set inner html getServis dengan button value
    ...
    konfirmasiDeleteServis.addEventListener('click', () => {
        const id = document.getElementById("Konfirmasi-delete-servis").value;

        konfirmasiDeleteServis.value = id;
