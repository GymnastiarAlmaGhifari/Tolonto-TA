...
<section>
    <div id="modal_overlay_info" class="hidden absolute inset-0 bg-black bg-opacity-30 h-screen w-full flex justify-center items-start md:items-center pt-10 md:pt-0 z-50">

        <!-- modal -->
        <div id="modal_info" class="opacity-0 transform -translate-y-full scale-150  relative bg-neutral_800 h-[520px] w-[500px] rounded-2xl flex flex-col justify-center gap-4 transition-opacity transition-transform duration-300">
            <div class="flex flex-row justify-between items-center -mt-3">
                <div class="flex flex-row justify-start ml-[23px] mt-4 gap-3 mb-3">
                    ...
                    <h1 class="text-neutral_050 font-base font-noto-sans text-xl">Id <span id="id_servis"></span></h1>
                </div>
                <button onclick="openModalInfo(false)" class="bg-neutral_050 w-[30px] h-[30px] rounded-full mr-6 relative">
            ...
            <form action="servisSuperAdmin.php" method="post" class="flex flex-col items-center justify-center gap-4 -mt-2" enctype="multipart/form-data">
                <div class="relative z-0 w-11/12 ">
                    ...
                    <input disabled type="text" id="nama_barang" name="nama_barang" required class="block py-2.5 text-base text-neutral_900 bg-neutral_050 w-full h-14 rounded-2xl pl-3" />
                </div>
                <div class="relative z-0 w-11/12 ">
                ...
                <div class="flex flex-row gap-[42px] items-center justify-center w-full">
                    <a href="https://wa.me/62895387390033/?text-Hello" type="button" target="_blank" name="map" id="map" class="bg-error_600 text-neutral_050 shadow-elevation-light-2 w-5/12 h-12 rounded-2xl flex flex-row gap-3 justify-center items-center">
                    ...
                    <a href="https://wa.me/62895387390033/?text-Hello" type="button" target="_blank" name="whatsapp" id="whatsapp" class="bg-[#4FCF2F] text-neutral_050 shadow-elevation-light-2 w-5/12 h-12 rounded-2xl flex flex-row gap-3 justify-center items-center">
...
<script>
    const modal_overlay_info = document.querySelector('#modal_overlay_info');
    const modal_info = document.querySelector('#modal_info');
    const infoServis = document.querySelectorAll('#infoServis');
    const id_servis = document.querySelector('#id_servis');

    const openModalInfo = (value) => {
        const modalClInfo = modal_info.classList
        const overlayClInfo = modal_overlay_info

        if (value) {
            overlayClInfo.classList.remove('hidden')
            setTimeout(() => {
                modalClInfo.remove('opacity-0')
                modalClInfo.remove('-translate-y-full')
                modalClInfo.remove('scale-150')
            }, 100);
        } else {
            modalClInfo.add('-translate-y-full')
            setTimeout(() => {
                modalClInfo.add('opacity-0')
                modalClInfo.add('scale-150')
            }, 100);
            setTimeout(() => overlayClInfo.classList.add('hidden'), 300);
        }
    }
    openModalInfo(false)

    infoServis.forEach((item) => {
