        ...
        <div id="modal_editAdmin" class="opacity-0 transform -translate-y-full scale-150  relative bg-neutral_800 h-[520px] w-[500px] rounded-2xl flex flex-col justify-center gap-4 transition-opacity transition-transform duration-300">
            ...
            <form action="userSuperAdmin.php" method="post" class="flex flex-col items-center justify-center gap-4 mt-2" enctype="multipart/form-data">
                ...
                <div class="relative z-0 w-11/12">
                    <input type="password-edit" id="password-edit" name="password-edit" required class="block py-2.5 text-base text-neutral_900 bg-neutral_050 w-full h-14 rounded-2xl focus:pt-5 valid:pt-5 pl-16 peer" placeholder=" " />
                    ...
                    <svg width="24" onclick="showEdit()" id="showimg-edit" class="absolute right-5 cursor-pointer top-4 z-20" onclick="showPass()" aria-hidden="true" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
...
<script>
    ...
    const imginp = document.getElementById('image-Admin-edit');
    const prev = document.getElementById('preview-Admin-Edit');
    ...
    const openModaleditAdmin = (value) => {
        const modalClAdminEdit = modal_editAdmin.classList
        const overlayClAdminEdit = modal_overlay_editAdmin

        if (value) {
            overlayClAdminEdit.classList.remove('hidden')
            setTimeout(() => {
                modalClAdminEdit.remove('opacity-0')
                modalClAdminEdit.remove('-translate-y-full')
                modalClAdminEdit.remove('scale-150')
            }, 100);
        } else {
            modalClAdminEdit.add('-translate-y-full')
            setTimeout(() => {
                modalClAdminEdit.add('opacity-0')
                modalClAdminEdit.add('scale-150')
            }, 100);
            setTimeout(() => overlayClAdminEdit.classList.add('hidden'), 300);
        }
    }
