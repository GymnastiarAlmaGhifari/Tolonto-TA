...
class Database extends Config
{
    ...
    {
        $data = $this->user('manage', 'username', $username);
    ...
    public function get_data($username)
    {
        if ($this->cek_nama($username)) {
            return $this->user('manage', 'username', $username);
        } else {
            return false;
        }
    }
    ...
    {
        $data = $this->user('user', 'email', $username);
        if (empty($data)) return false;
        else return true;
    }
    ...
    public function get_data1($username)
    {
        if ($this->cek_nama1($username)) {
            return $this->user('user', 'email', $username);
        } else {
            return false;
        }
    }
    ...
    {
        $data = $this->user('user', 'cookies', $cookie);
    ...
    public function registerApi($username, $email, $password)
    {
        $data = $this->insert('user', 'username', $username, 'email', $email, 'password', $password);
        if ($data) {
            return true;
        } else {
            return false;
